 ...
 resource "aws_instance" "nlpfubuntu" {
  ...
  provisioner "local-exec" {
        command = "sleep 100; 
		   ...
		   sudo ansible-playbook -u ubuntu --private-key ${var.private_key} playbook-install-docker.yml; 
		   pip install docker-compose;
		   sed -i -e "s/XX_FRONT_ADDR_XX/${aws_instance.nlpfubuntu.public_ip}/g" .envback	
		   sed -i -e "s/XX_BACK_ADDR_XX/${aws_instance.nlpfubuntu.public_ip}/g" .envfront
		   sudo ansible-playbook -u ubuntu --private-key ${var.private_key} playbook-docker-compose.yml"
    }
